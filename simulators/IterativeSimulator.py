...
class IterativeSimulation:
  ...
  def sobkowicz_replication_vectorized(self, information_source, filter_strength = 0):
    ...
    for k in range(self.iterations):
        for i in range(self.number_agents):
            filter = [0 for _ in range(granularity)]
            for j in range(granularity):
                filter[j] = self.belief_old[i][j]*filter_strength + (1-filter_strength) * uniform
            filtered_input = normalize([true_seek[j]*filter[j] for j in range(granularity)])
            self.beliefs[i] = self.update_belief(filtered_input, self.belief_old[i], self.beliefs_mean[i][k], i)
            sum = 0
            for j in range(granularity):
                sum += self.beliefs[i][j]*granular[j]
            self.beliefs_mean[i][k + 1] =  round(sum, 3)
  ...
  def update_belief(self, input, belief_old, previous_belief_mean, agent):
    ...
    if (random.uniform(0, 1)  < self.assimilation):
        belief = [0 for _ in range(granularity)]
        for j in range(granularity):
            belief[j] = belief_old[j]*input[j]
